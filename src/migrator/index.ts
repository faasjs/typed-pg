...
export class Migrator {
  ...
  constructor(options: {
    ...
    if (!existsSync(this.folder))
      throw Error(`Migration folder not found: ${this.folder}`)
  }
  ...
  async migrate() {
    ...
    for (const file of files) {
      ...
      try {
        ...
        await this.client.raw`INSERT INTO typed_pg_migrations (name, migration_time) VALUES (${name}, NOW())`
      } catch (error) {
  ...
  async up() {
    ...
    const migrations = await this.client.raw`SELECT * FROM typed_pg_migrations ORDER BY migration_time DESC LIMIT 1`
